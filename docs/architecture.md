...
DataTransferObject (DTOs for short) have a specific nomenclature to help keep them sorted.

`Requests` originate from the client
`Responses` originate on the server in response to a request.
`Commands` originate from the server for any reason that it desires.

In the chat handler it works something like this:

The client sends a message request
...
The server receives the request in the hub, issues commands to relevant connections
...
```csharp
    ...
    public async Task<ChatSendMessageResponse> SendChatMessage(ChatSendMessageRequest request)
    {
        ...
        // Returns the response to the client with an empty payload
        return new ChatSendMessageResponse(ChatSendMessageEc.Success);
    }
```

As per the chat flow, you can see that you send the messages, enqueue the various commands as needed and finally returns a response to the original requester.

Note: Commands are allowed to be sent by the server at anytime and it is perfectly possible for a background task to periodically send messages to users _without_ a prior request. Such as a message of the day, an alert, prank by the admins, etc.
